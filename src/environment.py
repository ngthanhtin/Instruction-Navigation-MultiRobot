...
class Env():
    def __init__(self, is_training, train_env_id, test_env_id=2, visual_obs=False, num_scan_ranges=10):
        ...
        self.pub_cmd_vel = rospy.Publisher('robot1/cmd_vel', Twist, queue_size=10)
    ...
    def step(self, action, past_action):
        ...
        while data is None:
            try:
                data = rospy.wait_for_message('robot1/scan', LaserScan, timeout=5)
            except:
                pass
        ...
        while self.visual_obs == True and image is None:
            try:
                image = rospy.wait_for_message('/robot1/camera1/image_raw', Image, timeout=5)
                bridge = CvBridge()
                try:
