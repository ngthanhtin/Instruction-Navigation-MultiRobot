...
class Env():
    def __init__(self, is_training, train_env_id, test_env_id=2, num_agents=2, visual_obs=False, num_scan_ranges=10):
        ...
        #-------INSTRUCTION-------#
        self.all_instructions = self.get_instr("/home/tinvn/TIN/Robot_NLP_Code/rl-mapless-navigation/src/instructions_all.json")
        self.word_to_idx = self.get_word_to_idx()
        self.vocab_size = len(self.word_to_idx)
        self.dictionary = self.word_to_idx
    ...
    def get_word_to_idx(self):
        word_to_idx = {}
        for instruction_data in self.all_instructions:
    ...
    def getInstruction(self):
        self.instruction = "Go to the "
        if self.model_names[self.object_target_id] == "car_wheel":
            self.instruction += "wheel"
        elif self.model_names[self.object_target_id] == "cricket_ball":
            self.instruction += "ball"
        elif self.model_names[self.object_target_id] == "beer":
            self.instruction += "beer"
        elif self.model_names[self.object_target_id] == "cardboard_box":
            self.instruction += "box"
        elif self.model_names[self.object_target_id] == "first_2015_trash_can":
            self.instruction += "trash"
    ...
    def step(self, action_s, past_action_s):
        ...
        return state_s, [self.instruction, self.instruction], r_s, die_s, arrive_s
    ...
    def reset(self):
        ...
        return state_s, [self.instruction, self.instruction]
